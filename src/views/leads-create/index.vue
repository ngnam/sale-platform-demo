<template>
  <router-view v-slot="{ route }">
    <div class="node-container">
      <h2 class="nc_title font18">Tạo mới khách hàng</h2>

      <a-steps v-model:current="currentStep" class="mb20" @change="handleStepChange">
        <a-step title="Chọn loại khách hàng" />
      ...
      <div v-if="currentStep === 0">
        <a-card title="Loại khách hàng">
          <a-radio-group v-model:value="form.isOrganization">
...
import { LeadApi, LeadCreateReq } from '@/apis';
...
const defaultForm: LeadCreateReq = {
  // Basic
  fullName: '',
...
const form = ref<LeadCreateReq>(defaultForm);
...
const openPanelForField = async (fieldName: string) => {
  ...
  if (!form.value.isOrganization) {
    personalActiveKeys.value = [panel];
  } else {
...
const onFinish = async () => {
  ...
  try {
    isLoadding.value = true;
    // TODO: call API create lead
    var res = await LeadApi.create(form.value);
    if (res) {
      console.log('Tạo hồ sơ khách hàng', res)
      ...
      await resetToDefault(defaultForm);
